...
exports.findOne = async(req, res) => {
    ...
    try {
        ...
        res.status(400).json({status: false, data: err});
        console.log("Problem in findding user's product", username);
    }
};
...
exports.updateProduct = async(req, res) => {
    ...
    const product_quantity = req.bod.product.quantity;
    console.log("Update product for username ", username, product_id, product_quantity);

    try {
        const result = await User.updateOne(
            {username: username, "products._id": product_id}, // finds the index that products.id = the value we gave
            {
                $set: {
                    "products.$.quantity": product_quantity // based on the index of what we found, we update it
                }
            }
        );
        res.status(200).json({status: true, data: result});
